...
namespace Industry {
	...
	public class EconomyStatics {
		
		private List<Resource> resources;
		private List<Industry> industries;
		
		// This should be called by world controller and passed into all economy members and children that need it
		public EconomyStatics() {
		...
		public String listResourceUses(bool verbose) {
			...
			foreach(int resourceId in resourcesList) {
				...
				foreach(Industry industry in industries) {
					IntegerMap resourceMap = industry.getInputs();
					if(resourceMap.get(resource.getId()) > 0) {
						inputIndustries.Add(industry);
						inputs += resourceMap.get(resource.getId());
					}
					resourceMap = industry.getOutputs();
					if(resourceMap.get(resource.getId()) > 0) {
						outputIndustries.Add(industry);
						outputs += resourceMap.get(resource.getId());
					}
				}
				if(outputs + inputs == 0) {
			...
			if(unusedResources.Count > 0) {
				...
				for(int i = 0; i < unusedResources.Count; i++) {
					s.Append(unusedResources[i].getName());
